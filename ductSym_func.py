...
def rp(vel, velt=new_z_vfield(), om=new_z_vfield(), t=0.0):
...
def pres(vel, ub=0.0, p=new_z_vfield(), inplace=False):
    if not inplace:
        vel = vel.copy()
...
def calc(vel, dtmax, cf=0.5, Re=None, velt=new_z_vfield(), om=new_z_vfield(), p=new_z_vfield(), maxnstep=None, tmax=None, 
    ...
    if not ready: 
        velt,om = rp(vel, velt, om, time)
        velt,p = pres(velt, 0.0, p, True)
...
def fullread_dcp(fname):
    ...
    velt, om = rp(vel, time=t)
    velt, p = pres(velt, 0.0, inplace=True)    
